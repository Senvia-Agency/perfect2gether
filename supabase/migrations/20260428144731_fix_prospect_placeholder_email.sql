-- Fix: when a prospect has no email/phone, create the lead with empty string
-- instead of a fake placeholder address (prospect-{uuid}@placeholder.local).
...
BEGIN
  ...
  LOOP
    ...
    INSERT INTO public.leads (
      ...
      p_organization_id,
      COALESCE(NULLIF(TRIM(v_prospect.contact_name), ''), NULLIF(TRIM(v_prospect.company_name), ''), '(sem nome)'),
      COALESCE(NULLIF(TRIM(v_prospect.email), ''), ''),
      COALESCE(NULLIF(TRIM(v_prospect.phone), ''), ''),
      NULLIF(TRIM(v_prospect.company_name), ''),
      NULLIF(TRIM(v_prospect.nif), ''),
      v_prospect.annual_consumption_kwh,
      NULLIF(TRIM(v_prospect.observations), ''),
      v_assigned_to,
      'prospect',
